...
const Edit = ({id}) => {
  ...
  const fetchData = async () => {
    await axios
      .get(`https://66fe20446993693089572358.mockapi.io/api/Books/${id}`)
      .then((res) => setEdit(res.data))
      .catch((error) => console.log(error));
  };

  // formik data useEffect Hook :
  useEffect(() => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      await axios
        .put(
          `https://66fe20446993693089572358.mockapi.io/api/Books/${id}`,
          values
        )
        .then((res) => res.data)
        .catch((error) => console.log(error));
  ...
  return (
    <div className="container text-center mb-4">
      <form onSubmit={formik.handleSubmit}>
        {/* Update Button creration : */}
        <div>
          <button
            className="btn btn-success d-flex justify-content-end "
            type="submit"
          >
            <FaBookOpen className="p-1" size={"30px"} /> Update{" "}
          </button>
        </div>
